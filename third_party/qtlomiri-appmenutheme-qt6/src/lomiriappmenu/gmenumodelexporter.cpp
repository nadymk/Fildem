...
#include "registry.h"
#include "logging.h"
#include "lomiriappmenuextraactionhandler.h"
...
inline QString getActionString(QString label)
{
    ...
    label = label.replace(QRegularExpression("(&|_)"), "");
...
{
    qCDebug(lomiriappmenu, "Activate menu action '%s'", g_action_get_name(G_ACTION(action)));
    auto item = static_cast<LomiriPlatformMenuItem*>(user_data);
    item->activated();
}

static uint s_menuId = 0;

#define MENU_OBJECT_PATH "/com/lomiri/Menu/%1"

} // namespace


LomiriMenuBarExporter::LomiriMenuBarExporter(LomiriPlatformMenuBar * bar)
    : LomiriGMenuModelExporter(bar)
{
    qCDebug(lomiriappmenu, "LomiriMenuBarExporter::LomiriMenuBarExporter");

    connect(bar, &LomiriPlatformMenuBar::structureChanged, this, [this]() {
    ...
    connect(&m_structureTimer, &QTimer::timeout, this, [this, bar]() {
        ...
        Q_FOREACH(QPlatformMenu *platformMenu, bar->menus()) {
            ...
            LomiriPlatformMenu* gplatformMenu = static_cast<LomiriPlatformMenu*>(platformMenu);
            if (gplatformMenu) {
                // Sadly we don't have a better way to propagate a enabled change in a top level menu
                // than reseting the whole menubar
                connect(gplatformMenu, &LomiriPlatformMenu::enabledChanged, bar, &LomiriPlatformMenuBar::structureChanged);
            }
        }
    });

    connect(bar, &LomiriPlatformMenuBar::ready, this, [this]() {
        exportModels();
    });
}

LomiriMenuBarExporter::~LomiriMenuBarExporter()
{
    qCDebug(lomiriappmenu, "LomiriMenuBarExporter::~LomiriMenuBarExporter");
}

LomiriMenuExporter::LomiriMenuExporter(LomiriPlatformMenu *menu)
    : LomiriGMenuModelExporter(menu)
{
    qCDebug(lomiriappmenu, "LomiriMenuExporter::LomiriMenuExporter");

    connect(menu, &LomiriPlatformMenu::structureChanged, this, [this]() {
        m_structureTimer.start();
    });
...
LomiriMenuExporter::~LomiriMenuExporter()
{
    qCDebug(lomiriappmenu, "LomiriMenuExporter::~LomiriMenuExporter");
}

LomiriGMenuModelExporter::LomiriGMenuModelExporter(QObject *parent)
    : QObject(parent)
    ...
    , m_exportedModel(0)
    , m_exportedActions(0)
    , m_lomiriAppMenuExtraHandler(nullptr)
...
LomiriGMenuModelExporter::~LomiriGMenuModelExporter()
{
    unexportModels();
    clear();
...
void LomiriGMenuModelExporter::clear()
{
...
void LomiriGMenuModelExporter::timerEvent(QTimerEvent *e)
{
    ...
    if (it != m_reloadMenuTimers.end()) {
        LomiriPlatformMenu* gplatformMenu = it.key();
...
void LomiriGMenuModelExporter::exportModels()
{
    GError *error = nullptr;
    m_connection = g_bus_get_sync (G_BUS_TYPE_SESSION, nullptr, &error);
    if (!m_connection) {
        qCWarning(lomiriappmenu, "Failed to retrieve session bus - %s", error ? error->message : "unknown error");
        g_error_free (error);
        return;
    }
    ...
    if (m_exportedModel == 0) {
        ...
        if (m_exportedModel == 0) {
            qCWarning(lomiriappmenu, "Failed to export menu - %s", error ? error->message : "unknown error");
            g_error_free (error);
            error = nullptr;
        } else {
            qCDebug(lomiriappmenu, "Exported menu on %s", g_dbus_connection_get_unique_name(m_connection));
        }
    }
    ...
    if (m_exportedActions == 0) {
        ...
        if (m_exportedActions == 0) {
            qCWarning(lomiriappmenu, "Failed to export actions - %s", error ? error->message : "unknown error");
            g_error_free (error);
            error = nullptr;
        } else {
            qCDebug(lomiriappmenu, "Exported actions on %s", g_dbus_connection_get_unique_name(m_connection));
        }
    }

    if (!m_lomiriAppMenuExtraHandler) {
        m_lomiriAppMenuExtraHandler = new LomiriAppMenuExtraActionHandler();
        if (!m_lomiriAppMenuExtraHandler->connect(m_connection, menuPath, this)) {
            delete m_lomiriAppMenuExtraHandler;
            m_lomiriAppMenuExtraHandler = nullptr;
        }
    }
}

void LomiriGMenuModelExporter::aboutToShow(quint64 tag)
{
    LomiriPlatformMenu* gplatformMenu = m_submenusWithTag.value(tag);
    if (!gplatformMenu) {
...
// Unexport the model
void LomiriGMenuModelExporter::unexportModels()
{
    GError *error = nullptr;
    if (!m_connection) {
        qCWarning(lomiriappmenu, "Failed to retrieve session bus - %s", error ? error->message : "unknown error");
    ...
    if (m_lomiriAppMenuExtraHandler) {
        m_lomiriAppMenuExtraHandler->disconnect(m_connection);
        delete m_lomiriAppMenuExtraHandler;
        m_lomiriAppMenuExtraHandler = nullptr;
    }
...
GMenuItem *LomiriGMenuModelExporter::createSubmenu(QPlatformMenu *platformMenu, LomiriPlatformMenuItem *forItem)
{
    LomiriPlatformMenu* gplatformMenu = static_cast<LomiriPlatformMenu*>(platformMenu);
    ...
    bool enabled;
    if (forItem) {
        label = LomiriPlatformMenuItem::get_text(forItem).toUtf8();
        enabled = LomiriPlatformMenuItem::get_enabled(forItem);
    } else {
        label = LomiriPlatformMenu::get_text(gplatformMenu).toUtf8();
        enabled = LomiriPlatformMenu::get_enabled(gplatformMenu);
    }

    addSubmenuItems(gplatformMenu, menu);

    Q_FOREACH(QPlatformMenuItem *childItem, gplatformMenu->menuItems()) {
        LomiriPlatformMenuItem* gplatformMenuItem = static_cast<LomiriPlatformMenuItem*>(childItem);
        ...
        if (gplatformMenuItem->menu()) {
            connect(gplatformMenuItem, &LomiriPlatformMenuItem::enabledChanged, gplatformMenu, &LomiriPlatformMenu::structureChanged);
        }
        connect(gplatformMenuItem, &LomiriPlatformMenuItem::visibleChanged, gplatformMenu, &LomiriPlatformMenu::structureChanged);
    }
    ...
    connect(gplatformMenu, &LomiriPlatformMenu::structureChanged, this, [this, gplatformMenu]
        {
    ...
    connect(gplatformMenu, &LomiriPlatformMenu::destroyed, this, [this, tag, gplatformMenu]
        {
...
void LomiriGMenuModelExporter::addSubmenuItems(LomiriPlatformMenu* gplatformMenu, GMenu* menu)
{
    ...
    for (; iter != gplatformMenu->menuItems().end(); ++iter) {
        LomiriPlatformMenuItem* gplatformMenuItem = static_cast<LomiriPlatformMenuItem*>(*iter);
        if (!gplatformMenuItem) continue;

        // don't add a section until we have separator
        if (LomiriPlatformMenuItem::get_separator(gplatformMenuItem)) {
...
GMenuItem *LomiriGMenuModelExporter::createMenuItem(QPlatformMenuItem *platformMenuItem, GMenu *parentMenu)
{
    LomiriPlatformMenuItem* gplatformMenuItem = static_cast<LomiriPlatformMenuItem*>(platformMenuItem);
    if (!gplatformMenuItem) return nullptr;

    if (!LomiriPlatformMenuItem::get_visible(gplatformMenuItem))
        return nullptr;

    QByteArray label(LomiriPlatformMenuItem::get_text(gplatformMenuItem).toUtf8());
    QByteArray actionLabel(getActionString(LomiriPlatformMenuItem::get_text(gplatformMenuItem)).toUtf8());
    QByteArray shortcut(LomiriPlatformMenuItem::get_shortcut(gplatformMenuItem).toString(QKeySequence::NativeText).toUtf8());
...
// Returned GMenuItem must be cleaned up using g_object_unref
GMenuItem *LomiriGMenuModelExporter::createSection(QList<QPlatformMenuItem *>::const_iterator iter, QList<QPlatformMenuItem *>::const_iterator end)
{
...
void LomiriGMenuModelExporter::processItemForGMenu(QPlatformMenuItem *platformMenuItem, GMenu *gmenu)
{
    LomiriPlatformMenuItem* gplatformMenuItem = static_cast<LomiriPlatformMenuItem*>(platformMenuItem);
...
void LomiriGMenuModelExporter::addAction(const QByteArray &name, LomiriPlatformMenuItem *gplatformMenuItem, GMenu *parentMenu)
{
    disconnect(gplatformMenuItem, &LomiriPlatformMenuItem::checkedChanged, this, 0);
    disconnect(gplatformMenuItem, &LomiriPlatformMenuItem::enabledChanged, this, 0);
    ...
    bool checkable = LomiriPlatformMenuItem::get_checkable(gplatformMenuItem);

    GSimpleAction* action = nullptr;
    if (checkable) {
        bool checked = LomiriPlatformMenuItem::get_checked(gplatformMenuItem);
        ...
        // save the connection to disconnect in LomiriGMenuModelExporter::clear()
        propertyConnections << connect(gplatformMenuItem, &LomiriPlatformMenuItem::checkedChanged, this, updateChecked);
    } else {
    ...
    updateEnabled(LomiriPlatformMenuItem::get_enabled(gplatformMenuItem));
    // save the connection to disconnect in LomiriGMenuModelExporter::clear()
    propertyConnections << connect(gplatformMenuItem, &LomiriPlatformMenuItem::enabledChanged, this, updateEnabled);
